<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'PUT')
    {
        ...
        if( isset($input['id']) and
            ...
            $val = updatelocation($conn,$id,$location);
            $out->location = $val;
            $output = $output && $out;
            $someupdates = true;
        }        
    ...
    function update($conn,$id,$name,$location,$notes,$create_date,$modified_date,$delete_date)
    {
        $query = 'update restaurant set id=?,name=?,location=?,notes=?,create_date=?,modified_date=?,delete_date=? where id=?';
        $stmt = $conn->prepare($query);
        $stmt->bind_param('issssssi',$idIn,$nameIn,$locationIn,$notesIn,$create_dateIn,$modified_dateIn,$delete_dateIn,$idIn);
    ...
    function updatename($conn,$id,$name){
        $query = 'update restaurant set name=? where id=?';
        $stmt = $conn->prepare($query);
    ...
    function updatelocation($conn,$id,$location){
        $query = 'update restaurant set location=? where id=?';
        $stmt = $conn->prepare($query);
        $stmt->bind_param('si',$locationIn,$idIn);
        $locationIn = $location;
        $idIn = $id;
        $result = $stmt->execute();
        $stmt->close();
    ...
    function updatenotes($conn,$id,$notes){
        $query = 'update restaurant set notes=? where id=?';
        $stmt = $conn->prepare($query);
        $stmt->bind_param('si',$notesIn,$idIn);
        $notesIn = $notes;
        $idIn = $id;
        $result = $stmt->execute();
        $stmt->close();
    ...
    function updatecreate_date($conn,$id,$create_date){
        $query = 'update restaurant set create_date=? where id=?';
        $stmt = $conn->prepare($query);
    ...
    function updatemodified_date($conn,$id,$modified_date){
        $query = 'update restaurant set modified_date=? where id=?';
        $stmt = $conn->prepare($query);
    ...
    function updatedelete_date($conn,$id,$delete_date){
        $query = 'update restaurant set delete_date=? where id=?';
        $stmt = $conn->prepare($query);
