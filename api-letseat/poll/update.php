<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'PUT')
    {
        ...
        if( isset($input['id']) and
            ...
            $val = updateend_date($conn,$id,$end_date);
            $out->end_date = $val;
            $output = $output && $out;
            $someupdates = true;
        }        
    ...
    function update($conn,$id,$name,$notes,$create_date,$end_date,$modified_date,$delete_date)
    {
        $query = 'update poll set id=?,name=?,notes=?,create_date=?,end_date=?,modified_date=?,delete_date=? where id=?';
        $stmt = $conn->prepare($query);
        $stmt->bind_param('issssssi',$idIn,$nameIn,$notesIn,$create_dateIn,$end_dateIn,$modified_dateIn,$delete_dateIn,$idIn);
    ...
    function updatename($conn,$id,$name){
        $query = 'update poll set name=? where id=?';
        $stmt = $conn->prepare($query);
    ...
    function updatenotes($conn,$id,$notes){
        $query = 'update poll set notes=? where id=?';
        $stmt = $conn->prepare($query);
    ...
    function updatecreate_date($conn,$id,$create_date){
        $query = 'update poll set create_date=? where id=?';
        $stmt = $conn->prepare($query);
    ...
    function updateend_date($conn,$id,$end_date){
        $query = 'update poll set end_date=? where id=?';
        $stmt = $conn->prepare($query);
        $stmt->bind_param('si',$end_dateIn,$idIn);
        $end_dateIn = $end_date;
        $idIn = $id;
        $result = $stmt->execute();
        $stmt->close();
    ...
    function updatemodified_date($conn,$id,$modified_date){
        $query = 'update poll set modified_date=? where id=?';
        $stmt = $conn->prepare($query);
    ...
    function updatedelete_date($conn,$id,$delete_date){
        $query = 'update poll set delete_date=? where id=?';
        $stmt = $conn->prepare($query);
